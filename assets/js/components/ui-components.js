...
async function renderServices(containerId) {
    ...
    container.innerHTML = data.map((service, index) => {
        ...
        return `
            <div class="group relative flex flex-col p-8 rounded-3xl glass-panel hover:bg-white hover:shadow-primary-hover hover:-translate-y-2 hover:scale-[1.02] transition-all duration-human ease-spring animate-on-scroll reveal-up stagger-${(index % 3) + 1}">
                <div class="flex-shrink-0 w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary text-3xl group-hover:bg-primary group-hover:text-white transition-all duration-human ease-spring mb-8">
...
async function renderFeatures(containerId) {
    ...
    container.innerHTML = data.map((feature, index) => `
        <div class="flex gap-5 items-start p-8 rounded-3xl glass-panel hover:bg-white hover:shadow-primary-hover hover:-translate-y-2 transition-all duration-human ease-spring border border-white/5 group animate-on-scroll reveal-up stagger-${(index % 3) + 1}">
            <div class="flex-shrink-0 w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary text-2xl group-hover:bg-primary group-hover:text-white transition-all duration-human ease-spring">
...
async function renderTestimonials(containerId) {
    ...
    container.innerHTML = slides.map((chunk, slideIndex) => `
        ...
        return `
                    <div class="card card--case-study bg-card border border-border overflow-hidden h-full hover:shadow-primary-hover hover:-translate-y-2 transition-all duration-human ease-spring group animate-on-scroll stagger-${(index % 3) + 1}">
                        <div class="card__image-wrapper">
                            <div class="absolute inset-0">
                                <img src="${config.basePath}${item.image}" alt="${item.alt}" width="400" height="250" class="w-full h-full object-cover transition-transform duration-human ease-spring group-hover:scale-105" loading="lazy">
                            </div>
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-human ease-spring"></div>
                        </div>
