import"./app-BwCdPM4z.js";const g=(function(){let e={basePath:"./",activePage:"home"};function o(a){return a.startsWith("#")?e.activePage==="home"?a:`${e.basePath}index.html${a}`:a==="index.html"?`${e.basePath}index.html`:a==="servicos.html"?`${e.basePath}servicos.html`:!a.startsWith("http")&&!a.startsWith("tel:")&&!a.startsWith("mailto:")?`${e.basePath}${a}`:a}function t(){const a=document.createElement("header");a.id="header",a.className="header fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur z-50 border-b border-white/10 shadow-2xl shadow-black/5 transition-all duration-300";const s=e.activePage==="home"?"#":`${e.basePath}index.html`;a.innerHTML=`
        ...
        `;const r=document.getElementById("header-placeholder");r?r.replaceWith(a):document.body.prepend(a);const c=a.querySelector("#nav-toggle"),m=a.querySelector("#nav-menu");c&&m&&c.addEventListener("click",()=>{m.classList.toggle("hidden")})}function i(){const a=document.createElement("footer");a.id="contact",a.className="bg-[#0A0A0B] text-white pt-24 pb-12 border-t border-white/5";const s=new Date().getFullYear();a.innerHTML=`
        ...
        `;const r=document.getElementById("footer-placeholder");r?r.replaceWith(a):document.body.appendChild(a)}function n(){const a=document.createElement("div");a.className="float-buttons",a.innerHTML=`
        ...
        `,document.body.appendChild(r)}function l(){const a=document.createElement("div");a.innerHTML=`
    ...
    `).join(""))}function h(e){const o=document.querySelector(e);o&&(o.innerHTML=w.map((t,i)=>{const n=Array(t.stars).fill('<i class="fa-solid fa-star"></i>').join("");return`
        <div class="card card--case-study bg-card border border-border overflow-hidden h-full hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 ease-in-out group animate-on-scroll stagger-${i%3+1}">
            <div class="relative h-48 overflow-hidden">
                <img src="${d.basePath}${t.image}" alt="${t.alt}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy">
    ...
    `}).join(""))}function $(e={}){d={...d,...e},p(".services__grid"),u(".features__list"),f(".coverage__cities"),h(".testimonials__list")}const q={init:$,renderServices:p,renderFeatures:u,renderCoverageCities:f,renderTestimonials:h};g.init({basePath:"./",activePage:"services"});q.init({basePath:"./"});
